...
public class Machine {
	...
	public Machine(String nameIn, Food foodIn, int capacityIn) {
		...
		this.exec = Executors.newFixedThreadPool(capacityIn);
	 ...
	 */
	public void makeFood(CountDownLatch cdl) throws InterruptedException {
		//YOUR CODE GOES HERE...
		exec.execute(new CookAnItem(cdl, this));
	...
	private class CookAnItem implements Runnable {
		CountDownLatch cdl;
		Machine mac;
		public CookAnItem(CountDownLatch cdl, Machine m){
			this.cdl = cdl;
			this.mac=m;
			}
		public void run() {
			try {				
				...
				Simulation.logEvent(SimulationEvent.machineDoneFood(mac,mac.machineFoodType));
				cdl.countDown();
